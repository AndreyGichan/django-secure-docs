...
import { decryptLocalFileOnly, getMyEncryptedDEK } from "@/lib/api/documents"
...
export default function DecryptionPage() {
  ...
  const handleDecrypt = async () => {
    ...
    try {
      ...
      const documentId = header.documentId

      if (!documentId) {
        throw new Error("Document ID not found in encrypted file")
      }
      ...
      // a.href = url
      // a.download = encryptedFile!.name.replace(/\.enc$/, "")
      // a.click()
                  ...
                  <div className="flex flex-col gap-3">
                    {[
                      ...
                      },
                      {
                        ...
                        desc: "Загрузите файл вашего приватного ключа для расшифровки документа",
